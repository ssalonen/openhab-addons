 ...
 */
@NonNullByDefault
public class WriteCoilFromOpenClosedHandler extends WriteCoilFromNumberHandler {
    public WriteCoilFromOpenClosedHandler(int slaveId, WriteChannelConfiguration config,
    ...
    protected Optional<BigDecimal> preProcessCommand(Command command) {
        return applyOnlyIf(OpenClosedType.class, command, onOffCommand -> {
            return (config.inverted ^ (onOffCommand == OpenClosedType.CLOSED)) ? BigDecimal.ZERO : BigDecimal.ONE;
        });
    }
